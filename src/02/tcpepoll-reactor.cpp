...
int main(int argc, char *argv[]) {
  ...
  servaddr.sin_port = htons(atoi(
      argv[2])); // ! 注意这里是将服务端的 ip 地址转换为能在网络中传输的大端序
  ...
  ev.data.fd =
      listenfd; // 指定事件的自定义数据，会随着 epoll_wait()返回的时间一并返回
    ...
    for (int ii = 0; ii < infds; ii++) {
      ...
      if (evs[ii].events & EPOLLRDHUP) { // 对方已关闭
        ...
        if (evs[ii].data.fd ==listenfd) { // listenfd 有事件，表示有新客户端连接上来
          ...
          int clientfd = accept4(listenfd, (struct sockaddr *)&clientaddr, &len, SOCK_NONBLOCK); // ! 注意最后一个参数是 &len
